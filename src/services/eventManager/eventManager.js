...
export const eventManager = (err, result) => {
  ...
  if (aa_address === config.TOKEN_REGISTRY) {
    ...
    if (isReq) {
      const { messages } = body.unit;
      const payload = getAAPayload(messages);
      const reserveAsset = payload.reserve_asset in config.reserves ? config.reserves[payload.reserve_asset].name : symbol_reserve_asset || payload.reserve_asset.slice(0, 6) + "...";
